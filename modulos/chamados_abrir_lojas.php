...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$titulo || !$descricao || !$loja_destino || !$idSolicitante) {
        $erro = "❌ Preencha todos os campos obrigatórios.";
    } else {

        if (!in_array($loja_destino, $lojasIds)) {
        ...
        if (empty($erro)) {
            ...
            if (empty($erro)) {
                ...
                // ===============================
                // INSERIR CHAMADO (MODELO ANTIGO CORRETO)
                // ===============================
                $stmt = $conn->prepare("
                ...
                if ($stmt->execute()) {
                    ...
                    $_SESSION['sucesso'] =
                        "✔️ Chamado <strong>{$codigo}</strong> aberto com sucesso para a loja {$nomeDestino}.";

                    header("Location: chamados_lojas_publico.php");
                    exit;
...
<h2>➕ Abrir chamado para Loja</h2>
<p>Preencha os dados abaixo para registrar um chamado para outra loja.</p>
...
<form method="POST" class="form-chamado">
    ...
    <div class="botoes-acoes">
        <a class="btn-voltar" href="chamados_lojas_publico.php">🔙 Voltar</a>
