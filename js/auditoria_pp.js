console.log("auditoria_pp.js carregado");
console.log("Auditoria PP — motor finalizado!");
...
function carregarItens() {
    ...
    fetch(`/ajax/auditoria_pp_itens.php?loja_id=${loja_id}`)
        .then(res => res.json())
        .then(lista => {
...
function mostrarSlide(indice) {
    ...
    if (campoData && !campoData.value) {
        ...
        // formato aceito pelo input date
        campoData.value = `${yyyy}-${mm}-${dd}`;
    }

    // ⭐ Inicializar assinatura somente quando o slide final aparece
...
function carregarUltimasAuditorias() {
    ...
    fetch("/ajax/auditoria_pp_historico_lista.php?limite=10")
        .then(res => res.json())
        .then(lista => {
...
document.addEventListener("click", function(e) {
    ...
    // ⭐ Impede QUALQUER navegação automática
    e.preventDefault();
